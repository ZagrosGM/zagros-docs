...
| `ZAGROS_DB_WAIT_SECONDS` | `180` | کانتینرِ پنل پیش از اجرای مهاجرت‌ها و بالاآمدن، حداکثر این‌قدر ثانیه منتظرِ پاسخ‌گوشدنِ سرورِ SQL بالا می‌ماند. فقط برای MySQL/MariaDB/PostgreSQL معنا دارد — SQLite هرگز منتظر نمی‌ماند. |
...
با پایگاه‌دادهٔ مدیریت‌شده (`--database mysql|mariadb|postgresql`) کانتینرِ پنل
اول منتظرِ آن سرور می‌ماند (`ZAGROS_DB_WAIT_SECONDS`)، بعد اسکیما را مهاجرت
می‌دهد و بعد سرویس می‌دهد. اگر سرور هرگز پاسخ ندهد، پنل باز هم بالا می‌آید، لایهٔ
پلتفرم را در دسترس‌ناپذیر گزارش می‌کند (HTTP 503 روی مسیرهای Zagros) و به‌محضِ
در دسترس بودنِ پایگاه‌داده خودش بازیابی می‌شود — نیازی به ری‌استارت نیست. سروری که
با خطای *پیکربندی* پاسخ دهد (رمزِ اشتباه، پایگاه‌دادهٔ ناشناخته) منتظر گذاشته
نمی‌شود: خطا همان لحظه ثبت می‌شود تا `.env` را درست کنید.
